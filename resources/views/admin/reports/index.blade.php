...
<body class="transition-all duration-300 bg-slate-50 dark:bg-slate-950 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        {{-- รายการงวด --}}
        <div
            class="transition-all duration-300 premium-card bg-white dark:bg-slate-900 rounded-2xl shadow-xl dark:shadow-2xl p-8 border border-slate-200 dark:border-slate-800">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b-2 border-slate-200 dark:border-slate-700">
                            ...
                            <th class="px-6 py-4 text-center font-bold text-slate-900 dark:text-white">รางวัล 3 ตัวบน
                            </th>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($draw->is_announced)
                                        <span
                                            class="inline-block px-4 py-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 rounded-full text-sm font-semibold">
                                            ✓ ประกาศแล้ว
                                        </span>
                                    @else
                                        <span
                                            class="inline-block px-4 py-2 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 rounded-full text-sm font-semibold">
                                            ⏳ กำลังขาย
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($draw->is_announced)
                                        <span
                                            class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{{ $draw->result_3_top }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($draw->is_announced)
                                        <span
                                            class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $draw->result_2_top }}</span>
                                    @else
                                        <span class="text-slate-400 dark:text-slate-600">-</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($draw->is_announced)
                                        <span
                                            class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{{ $draw->result_2_bottom }}</span>
                                    @else
                                        <span class="text-slate-400 dark:text-slate-600">-</span>
                                    @endif
                                </td>
